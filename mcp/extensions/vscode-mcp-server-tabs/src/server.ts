...
import { registerTabTools } from './tools/tab-tools';
    ...
    public setupTools(): void {
        ...
        if (this.fileListingCallback) {
            ...
            if (this.toolConfig.tab) {
                registerTabTools(this.server);
                logger.info('MCP tab tools registered successfully');
            } else {
                logger.info('MCP tab tools disabled by configuration');
            }
        } else {
