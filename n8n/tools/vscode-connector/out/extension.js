...
async function connectToN8n() {
    try {
        ...
        // 创建客户端并测试连接
        n8nClient = new n8n_client_1.N8nClient(baseUrl, apiKey);
...
// 查看工作流详情
async function viewWorkflowDetails() {
    if (!n8nClient) {
        vscode.window.showErrorMessage('Please connect to n8n instance first.');
        return;
    }
...
async function showWorkflowDetails(workflow) {
    const panel = vscode.window.createWebviewPanel('n8nWorkflowDetails', `Workflow: ${workflow.name}`, vscode.ViewColumn.One, {});
    panel.webview.html = getWorkflowDetailsHtml(workflow);
}
// 显示执行详情
async function showExecutionDetails(execution) {
    const panel = vscode.window.createWebviewPanel('n8nExecutionDetails', `Execution: ${execution.id}`, vscode.ViewColumn.One, {});
    panel.webview.html = getExecutionDetailsHtml(execution);
}
// 检查配置并尝试连接
async function checkConfigurationAndConnect() {
    // 优先使用环境变量
...
// 生成工作流详情HTML
function getWorkflowDetailsHtml(workflow) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
...
// 生成执行详情HTML
function getExecutionDetailsHtml(execution) {
    return `
    <!DOCTYPE html>
    <html>
    <head>
